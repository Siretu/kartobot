...
void ff_test_term_timer_callback(void)
{
	Timer++;
}
...
/* Put Disk Status code */
void put_ds(DSTATUS rc){
	xprintf("rc=%u %s%s%s\n", (UINT) rc, (rc & STA_NOINIT) ? "STA_NOINIT " : "STA_INIT ", (rc & STA_NODISK) ? "STA_NODISK " : "", (rc & STA_PROTECT) ? "STA_PROTECT" : "");
}
	...
	for (;;) {
		...
		case '?': /* ? - Show usage */
			xputs(
					"md <address> [<len>] - Dump system memory\n"
						...
						" \n"
						"fi [<drive>] - Mount the volume\n"
						"fs [<drive>] - Show volume status\n"
					  ...
					  "fp - Show current directory\n"
						"fm <drive> <rule> <cluster size> - Create file system\n"
						"fz [<len>] - Change R/W block length for fr/fw/fx command\n"
						" \n"
						"t [<year> <mon> <mday> <hour> <min> [<sec>]] - Show and change time\n"
						" \n"
						"a <file> - Play wav or mpc audio file\n"
						" \n");
			break;
		...
		case 'd' :
			...
			case 'i' :	/* di <phy_drv#> - Initialize disk */
				if (!xatoi(&ptr, &p1)) break;
				put_ds((WORD)disk_initialize((BYTE)p1));
				break;
...
#endif
#if _FS_RPATH >= 2
			case 'p' :	/* fp - Show current directory ? */
				res = f_getcwd(Buff, sizeof(Buff));
				if(res == 0){
					xprintf("%s\n", Buff);
				}else{
					put_rc(res);
		...
		case 't' :	/* t [<year> <mon> <mday> <hour> <min> [<sec>]] */
			...
			Time_GetTime(&rtc);
			xprintf("%lu-%02lu-%lu %02lu:%02lu:%02lu\n", rtc.YEAR, rtc.MONTH, rtc.DOM, rtc.HOUR, rtc.MIN, rtc.SEC);
			break;

		case 'a' :{	/* a [<loop>] <file> */
			uint8_t loop = 0;
			if (xatoi(&ptr, &p1)) {//TODO find why this fails for 3 digit numbers
				loop = p1;
			}
			while (*ptr == ' ') ptr++;
			size_t len = strlen(ptr);
			if(len > 3){
				if(strcasecmp(ptr + len - 3,"wav") == 0){
					char *path = pvPortMalloc2(len);
					if(path == NULL) break;
					strcpy(path, ptr);

					Song s = {.type = A_WAV, .data = path, .loops = loop};
					Audio_PlaySong(s);
					xprintf("File %s added to audio queue with %d repetitions.\n", ptr, loop);
					break;
				}else if(strcasecmp(ptr + len - 3,"mpc") == 0){
					char *path = pvPortMalloc2(len);
					if(path == NULL) break;
					strcpy(path, ptr);

					Song s = {.type = A_MPC, .data = path, .loops = loop};
					Audio_PlaySong(s);
					xprintf("File %s added to audio queue with %d repetitions.\n", ptr, loop);
					break;
				}else{
					xprintf("Unknown audio format\n");
				}
