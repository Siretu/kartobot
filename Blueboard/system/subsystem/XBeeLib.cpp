...
/**
 * Returns the amount of data available for read using next()
 */
uint8_t ZBRxResponse::available() {
	return getDataLength() - position;
}

/**
 * Returns the next byte or 0 if no more data
 */
uint8_t ZBRxResponse::next() {
	if(position < getDataLength()){
	  return getData()[position++];
	}else{
	  //Default to 0 if no more data
	  return 0;
	}
}

uint8_t ZBRxResponse::getPosition(){
	return position;
}

void ZBRxResponse::setPosition(uint8_t pos){
	position = pos > getDataLength() ? getDataLength() : pos;
}
...
bool XBee::readPacket(uint16_t timeout) {
	unsigned long start = Time_Millis();

    while (uint16_t((Time_Millis() - start)) < timeout) {
...
ZBTxRequest::ZBTxRequest() : PayloadRequest(ZB_TX_REQUEST, DEFAULT_FRAME_ID, (uint8_t*)NULL, 0) {
...
RemoteAtCommandRequest::RemoteAtCommandRequest() : AtCommandRequest((uint8_t*)NULL, (uint8_t*)NULL, 0) {
	_remoteAddress16 = 0;
	_applyChanges = false;
...
RemoteAtCommandRequest::RemoteAtCommandRequest(uint16_t remoteAddress16, uint8_t *command) : AtCommandRequest(command, (uint8_t*)NULL, 0) {
	_remoteAddress64 = broadcastAddress64;
	_remoteAddress16 = remoteAddress16;
	_applyChanges = false;
	setApiId(REMOTE_AT_REQUEST);
...
RemoteAtCommandRequest::RemoteAtCommandRequest(XBeeAddress64 &remoteAddress64, uint8_t *command) : AtCommandRequest(command, (uint8_t*)NULL, 0) {
	_remoteAddress64 = remoteAddress64;
	_remoteAddress16 = ZB_BROADCAST_ADDRESS;
	_applyChanges = false;
	setApiId(REMOTE_AT_REQUEST);
...
//	std::cout << "checksum is " << static_cast<unsigned int>(checksum) << std::endl;
	...
	// send packet
	UART1_send(buffer, p - buffer);
}

uint8_t* XBee::escape(uint8_t d, uint8_t* buffer) {
