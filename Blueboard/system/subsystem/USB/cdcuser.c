...
__attribute__((section(".ram1"))) __attribute__((aligned(4))) unsigned char BulkBufIn  [USB_CDC_BUFSIZE];            // Buffer to store USB IN  packet
__attribute__((section(".ram1"))) __attribute__((aligned(4))) unsigned char BulkBufOut [USB_CDC_BUFSIZE];            // Buffer to store USB OUT packet
...
void CDC_BulkIn(void) {
  ...
  uint16_t size = USB_ToSend();
  size = USB_Get(&BulkBufIn[0], size > USB_CDC_BUFSIZE ? USB_CDC_BUFSIZE : size);

  // send over USB
  if (size > 0) {
  	USB_WriteEP (CDC_DEP_IN, &BulkBufIn[0], size);
  }
  else {
    CDC_DepInEmpty = 1;
  }
...
void CDC_BulkOut(void) {
  ...
  uint32_t size = USB_ReadEP(CDC_DEP_OUT, &BulkBufOut[0]);

  if(size > USB_CDC_BUFSIZE){
	  xprintfD("USB BulkOut buffer overflow %lu !\n", size);
  }

  // store data in a buffer to transmit it over serial interface
  USB_Put(&BulkBufOut[0], size);
}
...
unsigned short CDC_GetSerialState (void) {
  CDC_SerialState = 0;
  unsigned short temp = 0;//LPC_UART0->LSR & 0x1E;
