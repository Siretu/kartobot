...
#ifndef MSC_RO
__attribute__((section(".ram2"))) uint8_t MSC_Memory[MSC_MemorySize];  /* MSC RAM */
#endif
...
void MSC_MemoryRead (void) {
  ...
  USB_WriteEP(MSC_EP_IN, &MSC_Memory[Offset], n);
  Offset += n;
  Length -= n;
...
#ifndef MSC_RO
  for (n = 0; n < BulkLen; n++) {
    if (MSC_Memory[Offset + n] != BulkBuf[n]) {
      MemOK = FALSE;
      break;
    }
  }
...
void MSC_Inquiry (void) {
  ...
  BulkBuf[32] = ' ';           /* Product Revision Level */
  BulkBuf[33] = ' ';
  BulkBuf[34] = ' ';
  BulkBuf[35] = ' ';
