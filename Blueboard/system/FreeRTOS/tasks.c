...
#if ( configUSE_TRACE_FACILITY == 1 )
unsigned portBASE_TYPE uxTaskGetID( xTaskHandle const pxTask )
{
	tskTCB *task = prvGetTCBFromHandle( pxTask );
	return task->uxTCBNumber;
}
#endif
/*-----------------------------------------------------------*/

#if ( configUSE_TRACE_FACILITY == 1 )
unsigned long uxTaskGetRunTimeCounter( xTaskHandle const pxTask )
{
	tskTCB *task = prvGetTCBFromHandle( pxTask );
...
static xTaskHandle * const prvGetTaskByIDWithinSingleList( xList *pxList, unsigned portBASE_TYPE uxTCBNumber )
{
volatile tskTCB *pxNextTCB, *pxFirstTCB;
	...
	do
	{
		...
		if(pxNextTCB->uxTCBNumber == uxTCBNumber){
...
static xTaskHandle * const prvGetTaskByID( unsigned portBASE_TYPE uxTCBNumber )
	{
	unsigned portBASE_TYPE uxQueue;
	xTaskHandle * pxTask;

		/* This is a costly function, It leaves interrupts disabled for some time. */
			...
			do
			{
				...
				if( listLIST_IS_EMPTY( &( pxReadyTasksLists[ uxQueue ] ) ) == pdFALSE )
				{
					pxTask = prvGetTaskByIDWithinSingleList( ( xList * ) &( pxReadyTasksLists[ uxQueue ] ), uxTCBNumber );
					if(pxTask != NULL){
						return pxTask;
					}
				}
			...
			if( listLIST_IS_EMPTY( pxDelayedTaskList ) == pdFALSE )
			{
				pxTask = prvGetTaskByIDWithinSingleList( ( xList * ) pxDelayedTaskList, uxTCBNumber );
				if(pxTask != NULL){
					return pxTask;
				}
			}
			...
			if( listLIST_IS_EMPTY( pxOverflowDelayedTaskList ) == pdFALSE )
			{
				pxTask = prvGetTaskByIDWithinSingleList( ( xList * ) pxOverflowDelayedTaskList, uxTCBNumber );
				if(pxTask != NULL){
					return pxTask;
				}
			}
			...
			#if( INCLUDE_vTaskDelete == 1 )
			{
				if( listLIST_IS_EMPTY( &xTasksWaitingTermination ) == pdFALSE )
				{
					pxTask = prvGetTaskByIDWithinSingleList( ( xList * ) &xTasksWaitingTermination, uxTCBNumber );
					if(pxTask != NULL){
						return pxTask;
					}
				}
			...
			#if ( INCLUDE_vTaskSuspend == 1 )
			{
				if( listLIST_IS_EMPTY( &xSuspendedTaskList ) == pdFALSE )
				{
					pxTask = prvGetTaskByIDWithinSingleList( ( xList * ) &xSuspendedTaskList, uxTCBNumber );
					if(pxTask != NULL){
						return pxTask;
					}
				}
...
xTaskHandle * const pxTaskGetByID( unsigned portBASE_TYPE uxID )
	{
		/* This is a costly function, It leaves interrupts disabled for some time. */

		vTaskSuspendAll();

		xTaskHandle * const pxTask = prvGetTaskByID( uxID );
