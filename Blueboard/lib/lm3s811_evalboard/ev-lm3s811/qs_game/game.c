...
//
//*****************************************************************************
// mthomas: used as index and signed char: static char g_pcMissile[2];
static unsigned char g_pcMissile[2];
...
UpdateMissile(tBoolean bFire, tBoolean bDead)
{
    ...
    if(g_pcMissile[1] < 8)
    {
        ...
        //
        g_pucFrame[g_pcMissile[0]] ^= ulBit;
        if((g_pucFrame[g_pcMissile[0]] & ulBit) != ulBit)
        {
            g_pucFrame[g_pcMissile[0]] |= ulBit;
            ulX = g_pcMissile[0];
        }
