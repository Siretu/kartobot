...
/*=======================================================================*/
HCD_STATUS EHcdInitDriver(uint8_t HostID)
{
	EHciHostReset(HostID);
	return EHciHostInit(HostID);
}
HCD_STATUS EHcdDeInitDriver(uint8_t HostID)
{
...
HCD_STATUS EHcdRhPortEnable(uint8_t HostID, uint8_t PortNum)
{
	return HCD_STATUS_OK;
}
HCD_STATUS EHcdRhPortDisable(uint8_t HostID, uint8_t PortNum)
{
	return HCD_STATUS_OK;
}
HCD_STATUS EHcdRhPortReset(uint8_t HostID, uint8_t uPortNumber)
{
...
HCD_STATUS EHcdClearEndpointHalt(uint32_t PipeHandle)
{
	return HCD_STATUS_OK;
}
uint32_t   EHcdGetFrameNumber(uint8_t HostID)
{
	return EHCI_REG(HostID)->FRINDEX_H;
}
HCD_STATUS EHcdGetDeviceSpeed( uint8_t HostID, uint8_t PortNumber, HCD_USB_SPEED* DeviceSpeed ) 
{
	if ( EHCI_REG(HostID)->PORTSC1_H & EHC_PORTSC_CurrentConnectStatus) /* If device is connected */
	{
		*DeviceSpeed = (HCD_USB_SPEED) ( (EHCI_REG(HostID)->PORTSC1_H & EHC_PORTSC_PortSpeed) >> 26 );
		return HCD_STATUS_OK;
	}else
	{
 ...
 * Note: 
 **********************************************************************/
HCD_STATUS EHcdOpenPipe(uint8_t HostID,
						uint8_t DeviceAddr,
	...
	switch (TransferType) // TODO should unify more, perharps later
	{
		...
		case BULK_TRANSFER:
			...
			DisableAsyncSchedule(HostID);
 			InsertQhd( HcdAsyncHead(HostID), HcdQHD(HeadIdx) );
			EnableAsyncSchedule(HostID);
		break;

		case INTERRUPT_TRANSFER:
			if (DeviceSpeed == HIGH_SPEED) /* currently not support HS due to lack of device for testing */
			{
				ASSERT_STATUS_OK_MESSAGE(HCD_STATUS_TRANSFER_TYPE_NOT_SUPPORTED, "Highspeed Interrupt is not supported yet, due to lack of testing");
			}

			ASSERT_STATUS_OK ( AllocQhd(DeviceAddr, DeviceSpeed, EndpointNumber, TransferType, TransferDir, MaxPacketSize, Interval, Mult, HSHubDevAddr, HSHubPortNum, &HeadIdx) );
			//DisablePeriodSchedule(HostID);
			InsertQhd( HcdIntHead(HostID), HcdQHD(HeadIdx) );
			//EnablePeriodSchedule(HostID);
		break;

		case ISOCHRONOUS_TRANSFER:
#ifndef __TEST__
			if (DeviceSpeed == HIGH_SPEED || TransferDir == IN_TRANSFER) /* currently not support HS due to lack of device for testing */
			{
 ...
 * Note: 
 **********************************************************************/
HCD_STATUS EHcdClosePipe(uint32_t PipeHandle)
{
	uint8_t HostID, HeadIdx;
	HCD_TRANSFER_TYPE XferType;

	ASSERT_STATUS_OK( EHcdCancelTransfer(PipeHandle) );
	...
	switch (XferType)
	{
		...
		case INTERRUPT_TRANSFER:
			...
			/* DoorBell Handshake: Queue Head will only be free in AsyncAdvanceIsr */
			EHCI_REG(HostID)->USBCMD_H |= EHC_USBCMD_IntAsyncAdvanceDoorbell;
		break;
 ...
 * Note: 
 **********************************************************************/
HCD_STATUS EHcdCancelTransfer(uint32_t PipeHandle)
{
	uint8_t HostID, HeadIdx;
	HCD_TRANSFER_TYPE XferType;
		...
		for (i=0; i<FRAME_LIST_SIZE; i++) /*-- Foreach link in Period List Base --*/
		{
			...
			while ( isValidLink(pNextPointer->Link) && pNextPointer->Type != QHD_TYPE)
			{
				if (pNextPointer->Type == ITD_TYPE)	/*-- Highspeed ISO --*/
				{
					...
					{
						pItd->Transaction[0].Active = pItd->Transaction[1].Active =
						pItd->Transaction[2].Active = pItd->Transaction[3].Active =
						pItd->Transaction[4].Active = pItd->Transaction[5].Active =
						pItd->Transaction[6].Active = pItd->Transaction[7].Active = 0;

						pItd->Transaction[0].IntOnComplete = pItd->Transaction[1].IntOnComplete =
						pItd->Transaction[2].IntOnComplete = pItd->Transaction[3].IntOnComplete =
						pItd->Transaction[4].IntOnComplete = pItd->Transaction[5].IntOnComplete =
						pItd->Transaction[6].IntOnComplete = pItd->Transaction[7].IntOnComplete = 0;
	...
	}else /*-- Bulk / Control / Interrupt --*/
	{
		...
		if ( XferType != INTERRUPT_TRANSFER )
		{
			DisableAsyncSchedule(HostID);
		}

		/*---------- Deactivate all queued TDs ----------*/
		/*-- Foreach Qtd in Qhd --*/
 ...
 * Note: 
 **********************************************************************/
HCD_STATUS EHcdControlTransfer(uint32_t PipeHandle, const uint8_t* const pDeviceRequest, uint8_t* const buffer)
{
	...
	Datalength = ((struct HCD_DeviceRequest *) pDeviceRequest)->wLength;
	direction =  ((struct HCD_DeviceRequest *) pDeviceRequest)->bmRequestType & 0x80;
	...
	if (Datalength)
	{
		ASSERT_STATUS_OK ( QueueQTDs(&DataTdIdx, buffer, Datalength, direction ? IN_TRANSFER : OUT_TRANSFER, 1, 0) );
	}else
	{
		DataTdIdx = SetupTdIdx; /* Data TDs will be skipped */
	}
 ...
 * Note: 
 **********************************************************************/
HCD_STATUS EHcdDataTransfer( uint32_t PipeHandle, uint8_t* const buffer, uint32_t const length, uint32_t* const pActualTransferred )
{
	uint8_t HostID, HeadIdx;
	HCD_TRANSFER_TYPE XferType;
	uint32_t DataTdIdx;
...
#ifndef __TEST__
	/* wait for semaphore compete TDs */
	ASSERT_STATUS_OK ( WaitForTransferComplete(HeadIdx) );
#endif

	if (pActualTransferred)
	{
		*pActualTransferred = ExpectedLength;	/* Should be actual bytes transferred */
	}
	return HCD_STATUS_OK;
}


uint8_t EHcdGetBoundEndpointAddress(uint32_t PipeHandle)
{
	uint8_t HostID, HeadIdx;
	HCD_TRANSFER_TYPE XferType;

	ASSERT_STATUS_OK ( PipehandleParse(PipeHandle, &HostID, &XferType, &HeadIdx) );
	return HcdQHD(HeadIdx)->DeviceAddress | (HcdQHD(HeadIdx)->Direction == 1 ? 0x80 : 0x00);
}
...
// static HCD_STATUS InsertQhd(uint8_t HostID, uint32_t QhdIdx)
// {
// 	HcdQHD(QhdIdx)->Horizontal.Link = HcdAsyncHead(HostID)->Horizontal.Link;
// 	HcdAsyncHead(HostID)->Horizontal.Link = Align32( (uint32_t) HcdQHD(QhdIdx) );
// 	HcdAsyncHead(HostID)->Horizontal.Type = QHD_TYPE;
// 	return HCD_STATUS_OK;
// }
static HCD_STATUS InsertQhd(PHCD_QHD pPreQhd, PHCD_QHD pQhd)
{
	pQhd->Horizontal.Link = pPreQhd->Horizontal.Link;
	pPreQhd->Horizontal.Link = Align32( (uint32_t) pQhd);
	pPreQhd->Horizontal.Type = QHD_TYPE;
	return HCD_STATUS_OK;
}
...
static HCD_STATUS AllocQTD (uint32_t* pTdIdx, uint8_t* const BufferPointer, uint32_t xferLen, HCD_TRANSFER_DIR PIDCode, uint8_t DataToggle, uint8_t IOC)
{
	...
	if ((*pTdIdx) < HCD_MAX_QTD)
	{
		...
		HcdQTD(*pTdIdx)->BuferPointer[0] = (uint32_t) BufferPointer;
		...
		{
			HcdQTD(*pTdIdx)->BuferPointer[idx] = Align4k( HcdQTD((*pTdIdx))->BuferPointer[idx-1] ) + 0x1000 ;
...
static HCD_STATUS QueueQTDs (uint32_t* pTdIdx, uint8_t* dataBuff, uint32_t xferLen, HCD_TRANSFER_DIR PIDCode, uint8_t DataToggle, uint8_t InterruptOnComplete)
{
	return AllocQTD(pTdIdx, dataBuff, xferLen, PIDCode, DataToggle, InterruptOnComplete) ;
}
#endif
...
static HCD_STATUS QueueQTDs (uint32_t* pTdIdx, uint8_t* dataBuff, uint32_t xferLen, HCD_TRANSFER_DIR PIDCode, uint8_t DataToggle, uint8_t InterruptOnComplete)
{
	...
	while (xferLen > 0)
	{
		...
		{
			ASSERT_STATUS_OK ( AllocQTD(pTdIdx, dataBuff, TdLen, PIDCode, DataToggle, (xferLen==0) ? InterruptOnComplete : 0) );
			TailTdIdx = *pTdIdx;
		}
		else
		{
			uint32_t NewTdIDx;
			ASSERT_STATUS_OK ( AllocQTD(&NewTdIDx, dataBuff, TdLen, PIDCode, DataToggle, (xferLen==0) ? InterruptOnComplete : 0) );
...
static HCD_STATUS QueueITDs( uint8_t HostID, uint8_t IhdIdx, uint8_t* dataBuff, uint32_t xferLen )
{
	...
	uint32_t FrameIdx = ((EHCI_REG(HostID)->FRINDEX_H >> 3) + 1) % FRAME_LIST_SIZE;
	...
	while (xferLen > 0)
	{
		...
		HcdHsITD(TdIdx)->Horizontal.Link = EHCI_FRAME_LIST(HostID)[FrameIdx].Link;
		EHCI_FRAME_LIST(HostID)[FrameIdx].Link = Align32( (uint32_t) HcdHsITD(TdIdx) );
		EHCI_FRAME_LIST(HostID)[FrameIdx].Type = ITD_TYPE;
...
static HCD_STATUS QueueSITDs( uint8_t HostID, uint8_t HeadIdx, uint8_t* dataBuff, uint32_t xferLen)
{
	...
	while (xferLen)
	{
		...
		HcdSITD(TdIdx)->Horizontal.Link = EHCI_FRAME_LIST(HostID)[FrameIdx].Link;
		EHCI_FRAME_LIST(HostID)[FrameIdx].Link = Align32( (uint32_t) HcdSITD(TdIdx) );
		EHCI_FRAME_LIST(HostID)[FrameIdx].Type = SITD_TYPE;

		FrameIdx = (FrameIdx + HcdQHD(HeadIdx)->Interval) % FRAME_LIST_SIZE;
		dataBuff += TDLen;
	}
	return HCD_STATUS_OK;
}
...
/*==========================================================================*/
void	EHcdIrqHandler (uint8_t HostID)
{
	uint32_t IntStatus;
...
/*---------- Interrupt On Compete has occured, however we have no clues on which QueueHead it happened. Also IOC TD may be advanced already
So we will free all TD which is not Active (transferred already) ----------*/
...
static __INLINE void DisableSchedule( uint8_t HostID, uint8_t isPeriod )
{
	...
	if (EHCI_REG(HostID)->USBSTS_H & statusMask)
	{
		...
		while (EHCI_REG(HostID)->USBSTS_H & statusMask){}
	}
}
...
// TODO temporarily suppress unused warnnings for DisablePeriodSchedule & EnablePeriodSchedule
static void __attribute__ ((unused)) DisablePeriodSchedule( uint8_t HostID )
{
	DisableSchedule(HostID, 1);
}

static void __attribute__ ((unused)) EnablePeriodSchedule( uint8_t HostID )
{
