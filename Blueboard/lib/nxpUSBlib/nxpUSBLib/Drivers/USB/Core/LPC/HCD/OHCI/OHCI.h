...
#define MAX_STATIC_ED						3 /* Serve as list head */
...
#define ITD_MAX_XFER_LENGTH			0x2000
...
typedef struct st_HCD_EndpointDescriptor {	// 32 byte align
	...
	/*---------- Word 1 ----------*/
	uint32_t inUse			: 1;
	uint32_t ListIndex		: 7;	// 0->62 : Interrupt/ISO, 63: not used, 64: Control, 65: bulk
	uint32_t MaxXactPerITD		: 4;	/*-- Used by ISO Transfer --*/
	uint32_t FramePeriod	: 13;	/*-- Used by ISO Transfer --*/
	uint32_t 				: 0;	/* Force next member on next storage unit */
	/*---------- End Word 1 ----------*/

	__IO uint32_t status; 		// TODO will remove __IO after remove all HcdQHD function
	uint32_t semaphore;			/* Currently acts as Number of queued TDs */
	uint32_t TotalTransferCount; /* total transferred bytes of a usb request */
...
typedef struct st_HC_GTD {	// 16 byte align
	...
	/*---------- Word 4 ----------*/
	uint8_t* BufferEnd;
} ATTR_ALIGNED(16) HC_GTD, *PHC_GTD;
...
HCD_STATUS OHcdRhPortEnable(uint8_t HostID, uint8_t PortNum); 
HCD_STATUS OHcdRhPortDisable(uint8_t HostID, uint8_t PortNum);
