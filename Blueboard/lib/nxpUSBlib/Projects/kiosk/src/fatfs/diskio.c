...
)
{
	...
	else if (MS_Host_ReadDeviceBlocks(&FlashDisk_MS_Interface, 0, sector, count, 512, buff))
	{
...
)
{
	...
	else if (MS_Host_WriteDeviceBlocks(&FlashDisk_MS_Interface, 0, sector, count, 512, buff))
	{
		MS_Host_ResetMSInterface(&FlashDisk_MS_Interface);
		ErrorCode = RES_ERROR;
	}
