...
#ifdef __LPC11UXX__
		if (BlockAddress+i > DATA_11U_ACTUAL_RAM_SIZE/VIRTUAL_MEMORY_BLOCK_SIZE)
		{
			Endpoint_Null_Stream(VIRTUAL_MEMORY_BLOCK_SIZE, NULL);
			Endpoint_ClearOUT();
			continue;
		}
#endif // __LPC11UXX__
		
		Endpoint_Read_Stream_LE(&DiskImage[(BlockAddress + i)*VIRTUAL_MEMORY_BLOCK_SIZE],VIRTUAL_MEMORY_BLOCK_SIZE,NULL);
		Endpoint_ClearOUT();
	}
}
                                 ...
                                 uint16_t TotalBlocks)
{
	...
	for(i=0;i<TotalBlocks;i++)
	{
		...
		Endpoint_Write_Stream_LE(&DiskImage[(BlockAddress+i)*VIRTUAL_MEMORY_BLOCK_SIZE], VIRTUAL_MEMORY_BLOCK_SIZE,NULL);
		Endpoint_ClearIN();
	}
}
...
#ifdef __LPC11UXX__ /* due to lack of 11uxx memory */
DATA_11U_ACTUAL_RAM_SIZE
#else
