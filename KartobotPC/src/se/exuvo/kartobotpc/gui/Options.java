...
public class Options extends BasicTWLGameState {
	...
	// TWL
	private Widget w;
	private EditField remoteAddress;
	...
	public void enter(GameContainer container, StateBasedGame game) throws SlickException {
		...
		remoteAddress.setText(Settings.getStr("remoteAddress64"));
    ...
    protected void createRootPane() {
		...
		remoteAddress = new EditField();
		remoteAddress.setSize(160, 16);
		remoteAddress.setPosition(110, y);
		w.add(remoteAddress);
		...
		apply.addCallback(new Runnable() {
			...
			public void run() {
				Main.pick.play();
				Settings.set("remoteAddress64", remoteAddress.getText());
				...
				if(width != Settings.getInt("GUI.Width") || height != Settings.getInt("GUI.Height") || fullscreen != Settings.getBol("GUI.Fullscreen")){
					try {
						...
						((TWLStateBasedGame)GUI.game).getCurrentState().enter(GUI.container, GUI.game);
