...
import se.exuvo.kartobotpc.gui.Loading;
...
public class ResourceManager implements ResourceLocation{
	...
	public static void init(String jarLocation) throws SlickException {
		...
		if(data.exists()){
			for(File d : data.listFiles()){
				if(d.isDirectory()){
					for(File f : getAllFiles(d)){
						if(! f.getPath().contains(".svn")){
							addResource(f.getPath(), d.getName());
						}
					}
				}
			}
	...
	public static void addResource(String path, String folder){
		String name = path.substring(path.indexOf(File.separator)+1).replace("\\", "/");
		log.trace("Adding Resource: " + folder + ":" + name);
	...
	private static class DeferredMusic implements DeferredResource {
		...
		public void load() throws IOException {
			try {
				ResourceManager.music.put(name, new Music(path));
			} catch (SlickException e) {
				e.printStackTrace();
			}
		}
